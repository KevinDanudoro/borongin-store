...
import useCarouselPagination from "@/hooks/components/useCarouselPagination";
    ...
    <Carousel setApi={setApi} plugins={[Autoplay({ delay: 5000 })]}>
      <CarouselContent className="ml-4 first:ml-0 cursor-grab active:cursor-grabbing">
        {Array.from({ length: 5 }).map((_, index) => (
          <CarouselItem
            key={index}
            className="flex flex-col md:flex-row justify-between items-center gap-4 bg-foreground text-background w-full h-96 rounded-md mr-4 last:mr-0 px-10 md:px-16 py-8 select-none"
          >
